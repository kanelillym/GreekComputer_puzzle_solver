...
def read_from_plate(plate_number, pos_y, pos_x):
...
def read_position(pos_y, pos_x):
    ...
    target_plate = 4
    while(result == 0):
        result = read_from_plate(target_plate, pos_y, pos_x)
...
def spin_incrementally():
    ...
    while True:
        ...
        if plate_rotations[plate_to_rotate] >= SPOKE_COUNT: # The plate has been spun fully around, rotate the next one down
